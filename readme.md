<h1 align="center">Hi 👋, I'm Ankit Raj</h1>
<h3 align="center">A passionate Computer Science student from India 🇮🇳</h3>

---

### 👨‍💻 About Me
- 🎓 I am a Computer Science student
- 💡 Interested in **Software Development, Cloud Computing, and Problem Solving**
- 🌱 Currently learning **Data Structures, Web Development, and Cloud Technologies**
- 🚀 Always exploring new technologies and building projects
- 📚 Goal: To become a skilled developer and contribute to real-world projects

---

### 🛠️ Tech Stack
- 💻 Languages: C++, Java, Python  
- 🌐 Web: HTML, CSS, JavaScript  
- ⚙️ Tools: Git, GitHub, VS Code  
- ☁️ Basics of Cloud Computing (AWS/Azure)

---

### 📌 Projects
- 🔹 **Student Management System**  
  Basic system to manage student records using C++  

- 🔹 **Portfolio Website**  
  Personal website using HTML, CSS, JavaScript  

- 🔹 **Cloud Computing Case Study**  
  Study on cloud architecture for real-world applications  

---

### 📈 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=radical" alt="stats"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_USERNAME&theme=radical" alt="streak"/>
</p>

---

### 🌱 Currently Learning
- Data Structures & Algorithms  
- Full Stack Development  
- Cloud Platforms (AWS / Azure)

---

### 🤝 Connect with Me
- 📧 Email: your-email@example.com  
- 💼 LinkedIn: https://linkedin.com/in/your-profile  

---

### ⚡ Fun Fact
- 💭 I believe consistency is the key to success!

